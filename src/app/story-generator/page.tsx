...
/* ---------- Page principale ---------- */ export default function UltimateNCPGenerator() { const [tab, setTab] = useState('story');

/* ---------- STRUCTURE ---------- */ const [story, setStory] = useState({ prompt: '', title: '', duration: '', chapters: '', tomes: '', genre: [] as string[], love: [] as string[], event: [] as string[], eventType: [] as string[], animals: [] as string[], animalRole: [] as string[], creativity: '', audience: '', ageRange: '', diversity: [] as string[], perspective: [] as string[], style: [] as string[], tone: [] as string[], chronology: '', forbiddenWords: '', creativityLevel: '', authorStyle: '', avoidRepetition: false, customStyle: '', narration: [] as string[], ending: [] as string[], cliffhanger: [] as string[], tensionLevel: '', allowDeath: [] as string[], allowSex: [] as string[], opening: [] as string[], supportType: '', commercialGoal: '', splitAudio: false, seo: false, });

const [character, setCharacter] = useState({ name: '', nickname: '', age: '', gender: '', appearance: '', personality: '', backstory: '', motivation: '', conflict: '', relationships: '', profession: '', skills: '', fatalFlaw: '', desire: '', fear: '', contextBirth: '', education: '', childhood: '', family: '', trauma: '', shortGoals: '', longGoals: '', obstacles: '', dreams: '', secrets: '', internalConflict: '', externalConflict: '', stressSources: '', friends: '', enemies: '', mentors: '', growthArc: '', dialogueStyle: '', residence: '', culturalContext: '', economicStatus: '', });

const [chapters, setChapters] = useState({ count: '1', structure: '', pacing: '', midpoint: '', climax: '', resolution: '', cliffhangers: false, flashbacks: false, multiTimeline: false, globalArc: [] as string[], arcType: [] as string[], timelineStyle: '', scenesPerChapter: '', acts: '', });

const [links, setLinks] = useState({ loyaltyConflict: false, redemptionQuest: false, raceAgainstTime: false, betrayal: false, familySecret: false, forbiddenLove: false, powerCorruption: false, survival: false, rivalry: false, truthQuest: false, intrigueLevel: [] as string[], genealogy: '', });

const [locations, setLocations] = useState({ urban: [] as string[], rural: [] as string[], coastal: [] as string[], fantasy: [] as string[], historical: [] as string[], futuristic: [] as string[], mystical: [] as string[], industrial: [] as string[], transport: [] as string[], leisure: [] as string[], });

const [themes, setThemes] = useState({ general: [] as string[], sport: [] as string[], erotic: [] as string[], professional: [] as string[], });

const [customization, setCustomization] = useState({ creativityLevel: '', style: [] as string[], tone: [] as string[], death: [] as string[], sex: [] as string[], opening: [] as string[], });

const [result, setResult] = useState(''); const [loading, setLoading] = useState(false); const [nodes, setNodes] = useState<any[]>([]); const [edges, setEdges] = useState<any[]>([]);

/* ---------- Mind map ReactFlow ---------- */ useEffect(() => { const initialNodes = [ { id: '1', position: { x: 250, y: 5 }, data: { label: 'Histoire' } }, ];
...
/* ---------- UTILS ---------- */ const handleMulti = (setter: any, key: string) => (value: string | string[]) => setter((prev: any) => ({ ...prev, [key]: value }));

const suggest = async (label: string, setter: any) => { try { const res = await fetch('/api/openrouter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: Suggère une idée pour : ${label}, maxTokens: 80 }), }); const data = await res.json(); setter(data.result?.trim() || ''); } catch { alert('Erreur suggestion'); } };

const generateAll = async () => { if (!story.prompt) return alert('Entrez une idée'); setLoading(true); const payload = { story, character, chapters, links, locations, themes, customization }; try { const res = await fetch('/api/openrouter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: Génère une histoire NCP complète : ${JSON.stringify(payload, null, 2)}, maxTokens: 3500 }), }); const data = await res.json(); setResult(data.result || ''); } finally { setLoading(false); } };

/* ---------- OPTIONS ---------- */ const genres = ["Education","Original","Classique","Humour","Science-fiction","Space opera","Extra-terrestre","Dystopie","Uchronie","Steampunk","Action","Thriller","Horreur","Réaliste","Biographie","Fiction","Non-fiction","Drame","Mystère","Voyage dans le temps","Bataille","Comédie","Kawaii","Magie","Mecha Battle","Fantasy","Aventure","Vengeance","Samouraï","Ninja","Kpop","Suspense","Guérison","Émotion","Superpouvoirs","Crime","Vie quotidienne","Compétition","Historique","Épique","Guerre","Sports"]; const loveOptions = ["Intrigue","Romance légère","Romance intense","Romance complexe","New romance","Dark Romance","SM Romance","BDSM Romance","Passion","Passion amoureuse","Rupture amoureuse","Echangisme","Boys' Love","Girls’ Love","Triangle amoureux","Amour impossible","Première amour","Erotique","Pornographique","Amour torride","Amour toxique","Amour tordu","amour amical","sex friend","amour à distance","amour virtuel","amour multiple","amour interdit","amour perdu","Amour polyamoureux","Amour asexuel"]; const events = ["Difficulté à l’école","Harcèlement","Sauver le monde","Sauver les autres","Rupture amicale","Triangle amoureux","Coup de foudre","Malentendu","Explorer l'inconnu","Éveil du héros","Amnésie","Mystère de l'identité","Travail d'équipe","Voyage dans le temps et l'espace","Bataille fatale","Trahison par la famille","Secret de famille","Événement mystérieux","Ascension du méchant","Civilisation perdue","Monde virtuel","Survie à l'apocalypse","Progression professionnelle","Croissance dans l'adversité","Chassez les rêves","Vie ordinaire","Ment","Enquête sur un événement","Meurtre","Amour perdu et réuni","Test familial","Amour sado-masochiste","Douceur","Développement du personnage","Amélioration des compétences","Retrait","Lutte pour le pouvoir","Guerre des gangs","Poursuite et évasion","Complot politique","Choc des civilisations","Mystère historique","Lutte commerciale","Adieu","Triste fin","Premier amour","Regret irréparable","Gestion d'entreprise","Croissance difficile","Côté obscur de l'industrie","Découverte scientifique","Catastrophe naturelle","Révolution technologique"]; const creativityLevels = ["Conservateur","Équilibré","Inventif","Expérimental","Traditionnel","Innovant","Classique","Moderne","Avant-Gardiste","Éclectique"]; const tones = ["Sérieux","Humoristique","Ironique","Sarcastique","Mélancolique","Optimiste","Pessimiste","Neutre","Passionné","Réfléchi"]; const styles = ["Réaliste","Poétique","Journalistique","Épistolaire","Stream of Consciousness","Minimaliste","Descriptif","Dialogué","Lyrical","Satirique"]; const locationsUrban = ["Ville Moderne","Quartier Historique","Banlieue Résidentielle","Centre Commercial","Ghetto Urbain"]; const locationsRural = ["Ferme Agricole","Village de Campagne","Forêt et Nature","Montagnes et Collines","Lacs et Rivières"]; const locationsCoastal = ["Plage Tropicale","Port de Pêche","Île Déserte","Station Balnéaire","Falaises et Grottes Marines"]; const locationsFantasy = ["Royaume Magique","Univers Parallèle","Monde Post-Apocalyptique","Espace et Planètes Lointaines","Monde Sous-Marin"]; const locationsHistorical = ["Égypte Antique","Moyen Âge Européen","Renaissance Italienne","Guerre Mondiale","Révolution Industrielle"]; const locationsFuturistic = ["Ville Cyberpunk","Colonie Spatiale","Monde Post-Humain","Terre Dévastée","Utopie Technologique"]; const locationsMystical = ["Temple Ancien","Forêt Enchantée","Monastère Isolé","Cimetière Hanté","Jardin Secret"]; const locationsIndustrial = ["Usine Automobile","Laboratoire de Recherche","Centre de Données","Chantier de Construction","Mine Souterraine"]; const locationsTransport = ["Train de Voyage","Navire de Croisière","Station Spatiale","Aéroport International","Route de Campagne"]; const locationsLeisure = ["Parc d'Attractions","Stade Sportif","Casino","Festival de Musique","Centre de Loisirs"]; const themesGeneral = ["Romans et Littérature","Romans d'amour","Science-fiction","Fantasy","Romans historiques","Thrillers psychologiques","Romans policiers","Littérature jeunesse","Romans de développement personnel","Romans d'aventure","Romans biographiques","Bandes Dessinées et Mangas","Shonen","Shojo","Seinen","Josei","Bandes dessinées franco-belges","Comics américains","Webtoons","Livres pour Enfants et Ados","Albums illustrés","Contes et fables","Livres éducatifs","Romans pour adolescents","Livres d'activités et de coloriage","Livres de développement personnel pour ados","Livres de science-fiction pour jeunes","Livres d'aventure pour enfants","Livres sur l'amitié et les relations","Livres de mystère pour jeunes détectives","Livres Scolaires et Éducatifs","Manuels de mathématiques","Livres de grammaire et de conjugaison","Cahiers d'exercices","Livres de préparation aux examens","Manuels de sciences","Livres d'histoire et de géographie","Livres de langues étrangères","Livres de philosophie","Livres de littérature","Livres de méthodologie et d'organisation","Santé et Bien-être","Loisirs et Culture","Développement Personnel et Professionnel","Technologie et Informatique","Gestion de Projet et Méthodologies Agiles","Marketing et Ventes","Finance et Comptabilité","Santé et Bien-être au Travail","Entrepreneuriat et Innovation","Droit et Réglementation","Langues et Communication","Éducation et Formation Continue","Littérature Pornoérotique"]; const themesSport = ["Guides d'Entraînement","Stratégies de football","Techniques de tennis","Sports de plein air","Sports collectifs","Sports individuels","Sports adaptés","Sports extrêmes"]; const themesErotic = ["Romans Érotiques Contemporains","Nouvelles érotiques","Érotique LGBTQ+","Guides érotiques","Fantasmes et fétichismes","Rencontres en ligne","Voyages érotiques"]; const themesProfessional = ["Développement Personnel","Technologie","Gestion de projet","Marketing","Finance","Santé au travail","Entrepreneuriat","Droit","Langues","Éducation"];

return (

Générateur Ultime NCP
Toutes les options PDF intégrées + IA
    ...
    <Tabs value={tab} onValueChange={setTab}>
      ...
      <TabsContent value="story">
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <SelectMulti label="Créativité" value={story.creativity} onChange={(v) => setStory({ ...story, creativity: v })} options={creativityLevels} singleSelection={true} />
            <SelectMulti label="Public" value={story.audience} onChange={(v) => setStory({ ...story, audience: v })} options={["Adulte", "Adolescent", "Enfant"]} singleSelection={true} />
            <SelectMulti label="Tranche d'âge" value={story.ageRange} onChange={(v) => setStory({ ...story, ageRange: v })} options={["3-5 ans", "6-8 ans", "9-10 ans", "11-13 ans", "14-16 ans", "17-20 ans", "21-25 ans", "26-35 ans", "36-50 ans", "51-65 ans", "65+"]} singleSelection={true} />
            ...
            <SelectMulti label="Support final" value={story.supportType} onChange={(v) => setStory({ ...story, supportType: v })} options={["Ebook", "Livre audio", "Podcast", "Vidéo", "Article de blog", "Autre"]} singleSelection={true} />
            <SelectMulti label="Objectif commercial" value={story.commercialGoal} onChange={(v) => setStory({ ...story, commercialGoal: v })} options={["Commercialisable", "Création pure", "Autre"]} singleSelection={true} />
            ...
            <SelectMulti label="Style narratif" value={story.narration} onChange={handleMulti(setStory, 'narration')} options={[
              'Narratif classique','Descriptif','Dialogué','Monologue intérieur','Stream of consciousness','Épistolaire','Journal intime','Chronique',
            ]} />
            <SelectMulti label="Type de Récit" value={story.eventType} onChange={handleMulti(setStory, 'eventType')} options={[
              'Récit interactif','Récit immersif','Récit poétique','Récit lyrique','Récit dramatique','Récit humoristique','Récit satirique','Récit ironique','Récit tragique',
            ]} />
          </CardContent>
        </Card>
      </TabsContent>
      ...
      <TabsContent value="character">
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <SelectMulti label="Genre" value={character.gender} onChange={(v) => setCharacter({ ...character, gender: v })} options={["Masculin", "Féminin", "Non-binaire"]} singleSelection={true} />
      ...
      <TabsContent value="chapter">
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <SelectMulti label="Longueur de l'histoire" value={story.duration} onChange={(v) => setStory({ ...story, duration: v })} options={[
              'Micro Fiction (100 à 500 mots)','Courte Nouvelle (1000-2500 mots)','Moyenne Nouvelle (2501-5000 mots)','Longue Nouvelle (5001-7500 mots)','Roman court (7501-10 000 mots)','Roman Moyen (10 001-30 000 mots)','Roman Long (30 001+ mots)',
            ]} singleSelection={true} />
            <div>
              <label className="block text-sm font-medium mb-1">Style narratif</label>
              <div className="grid grid-cols-2 gap-2">
                <Input type="number" placeholder="Scènes par Chapitre" onChange={(e) => setChapters({ ...chapters, scenesPerChapter: e.target.value })} />
                <Input type="number" placeholder="Nombre de Chapitres" value={chapters.count} onChange={(e) => setChapters({ ...chapters, count: e.target.value })} />
                <Input type="number" placeholder="Nombre d'Actes" onChange={(e) => setChapters({ ...chapters, acts: e.target.value })} />
                <Input type="number" placeholder="Nombre de Tomes" onChange={(e) => setStory({ ...story, tomes: e.target.value })} />
              </div>
            </div>
            <SelectMulti label="Structure" value={chapters.structure} onChange={(v) => setChapters({ ...chapters, structure: v })} options={["Acte 3", "Acte 5", "Hero’s Journey", "Freytag", "Save the Cat!"]} singleSelection={true} />
            <SelectMulti label="Pacing" value={chapters.pacing} onChange={(v) => setChapters({ ...chapters, pacing: v })} options={["Lent", "Modéré", "Rapide", "Variable"]} singleSelection={true} />
            <SelectMulti label="Timeline style" value={chapters.timelineStyle} onChange={(v) => setChapters({ ...chapters, timelineStyle: v })} options={[
              'Récit Linéaire','Récit Non linéaire','Flashbacks','Anticipations','Récit enchâssé','Timelines parallèles','Sauts temporels','Boucles temporelles','Récit circulaire','Récit en spirale','Structure en miroir','Structure en actes','Structure pyramidale','Structure en cascade','Structure modulaire','Structure fragmentée','Structure kaléidoscopique','Structure labyrinthique',
            ]} singleSelection={true} />
      ...
      <TabsContent value="custom">
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <SelectMulti label="Niveau de créativité" value={customization.creativityLevel} onChange={(v) => setCustomization({ ...customization, creativityLevel: v })} options={creativityLevels} singleSelection={true} />
            <SelectMulti label="Style d’écriture" value={customization.style} onChange={handleMulti(setCustomization, 'style')} options={[
              'Sérieux','Humoristique','Ironique','Sarcastique','Mélancolique','Optimiste','Pessimiste','Neutre','Passionné','Réfléchi','Dramatique','Comique','Tragique','Romantique','Sexy','Mystérieux','Suspense','Émotionnel','Philosophique','Politique','Social','Culturel','Historique','Scientifique','Fantastique','Horrifique','Érotique','Pornographique','Éducationnel','Inspirant','Motivant','Révélateur','Provocateur','Révolutionnaire','Traditionnel','Moderne','Classique','Avant-gardiste','Expérimental','Toxique',
            ]} />
            ...
            <Input placeholder="Style d'auteur à imiter" value={story.authorStyle} onChange={(e) => setStory({ ...story, authorStyle: e.target.value })} />
            <Textarea placeholder="Mon style d’écriture (2000 caractères max)" maxLength={2000} onChange={(e) => setStory({ ...story, customStyle: e.target.value })} />
            <label className="flex items-center gap-2">
