...
const SelectMulti = ({ label, value, onChange, options }: any) => {
  ...
  const availableOptions = options.filter((opt: string) => !selected.includes(opt));
    ...
    <div className="space-y-2">
      ...
      >
        <SelectTrigger className="w-full">
          <SelectValue placeholder={`Ajouter ${label.toLowerCase()}`} />
        </SelectTrigger>
        <SelectContent>
...
export default function UltimateNCPGenerator() {
  ...
  /* ---------- Mind map ReactFlow ---------- */
  useEffect(() => {
    const initialNodes = [{ id: '1', position: { x: 250, y: 5 }, data: { label: 'Histoire' } }];
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="story">
            <Card>
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <SelectMulti label="Genre" value={story.genre} onChange={(v) => setStory({ ...story, genre: v })} options={genres} />
                <SelectMulti label="Amours" value={story.love} onChange={(v) => setStory({ ...story, love: v })} options={loveOptions} />
                <SelectMulti label="Événements" value={story.event} onChange={(v) => setStory({ ...story, event: v })} options={events} />
                <SelectMulti label="Animaux" value={story.animals} onChange={(v) => setStory({ ...story, animals: v })} options={["ferme","jungle","zoo","domestique","dinosaure","sauvage","légendaire","inventé","mythologique","fantastique","dragon","licorne","créatures marines","créatures volantes"]} />
                ...
                <SelectMulti label="Diversité" value={story.diversity} onChange={(v) => setStory({ ...story, diversity: v })} options={["LGBTQ+","Personnes handicapées","Communautés culturelles","genre","ethnicité","religion","philosophie"]} />
                <SelectMulti label="Perspective" value={story.perspective} onChange={(v) => setStory({ ...story, perspective: v })} options={["Première personne","Deuxième personne","Troisième personne","Omniscient","Flux de conscience","Épistolaire","Perspective multiples","Interactif","Observateur","Œil de caméra","Narrateur peu fiable"]} />
                ...
                <SelectMulti label="Style narratif" value={story.narration} onChange={(v) => setStory({ ...story, narration: v })} options={[
                  'Narratif classique','Descriptif','Dialogué','Monologue intérieur','Stream of consciousness','Épistolaire','Journal intime','Chronique',
                ]} />
                <SelectMulti label="Type de Récit" value={story.eventType} onChange={(v) => setStory({ ...story, eventType: v })} options={[
          ...
          <TabsContent value="chapter">
            <Card>
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SelectMulti label="Longueur de l'histoire" value={[story.duration]} onChange={(v) => setStory({ ...story, duration: v[0] || '' })} options={[
                ...
                <SelectMulti label="Timeline style" value={[chapters.timelineStyle]} onChange={(v) => setChapters({ ...chapters, timelineStyle: v[0] || '' })} options={[
                  'Récit Linéaire','Récit Non linéaire','Flashbacks','Anticipations','Récit enchâssé','Timelines parallèles','Sauts temporels','Boucles temporelles','Récit circulaire','Récit en spirale','Structure en miroir','Structure en actes','Structure pyramidale','Structure en cascade','Structure modulaire','Structure kaléidoscopique','Structure labyrinthique',
                ]} />
                <SelectMulti label="Global Arc" value={chapters.globalArc} onChange={(v) => setChapters({ ...chapters, globalArc: v })} options={["Voyage du Héros","Chute et Rédemption","Quête","Transformation","Découverte de Soi","Révolte","Survie","Ascension et Chute","Quête de Vérité","Réconciliation"]} />
                <SelectMulti label="Arc type" value={chapters.arcType} onChange={(v) => setChapters({ ...chapters, arcType: v })} options={["Arc principal","Arc secondaire","Sous-arc"]} />
          ...
          <TabsContent value="links">
            <Card>
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <SelectMulti label="Niveau d'intrigue" value={links.intrigueLevel} onChange={(v) => setLinks({ ...links, intrigueLevel: v })} options={["Intrigue principale","Intrigue secondaire","Sous-intrigue"]} />
          ...
          <TabsContent value="locations">
            <Card>
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SelectMulti label="Urbain" value={locations.urban} onChange={(v) => setLocations({ ...locations, urban: v })} options={locationsUrban} />
                <SelectMulti label="Rural" value={locations.rural} onChange={(v) => setLocations({ ...locations, rural: v })} options={locationsRural} />
                <SelectMulti label="Côtier" value={locations.coastal} onChange={(v) => setLocations({ ...locations, coastal: v })} options={locationsCoastal} />
                <SelectMulti label="Fantastique" value={locations.fantasy} onChange={(v) => setLocations({ ...locations, fantasy: v })} options={locationsFantasy} />
                <SelectMulti label="Historique" value={locations.historical} onChange={(v) => setLocations({ ...locations, historical: v })} options={locationsHistorical} />
                <SelectMulti label="Futuriste" value={locations.futuristic} onChange={(v) => setLocations({ ...locations, futuristic: v })} options={locationsFuturistic} />
                <SelectMulti label="Mystique" value={locations.mystical} onChange={(v) => setLocations({ ...locations, mystical: v })} options={locationsMystical} />
                <SelectMulti label="Industriel" value={locations.industrial} onChange={(v) => setLocations({ ...locations, industrial: v })} options={locationsIndustrial} />
                <SelectMulti label="Transport" value={locations.transport} onChange={(v) => setLocations({ ...locations, transport: v })} options={locationsTransport} />
                <SelectMulti label="Loisirs" value={locations.leisure} onChange={(v) => setLocations({ ...locations, leisure: v })} options={locationsLeisure} />
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="themes">
            <Card>
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SelectMulti label="Général" value={themes.general} onChange={(v) => setThemes({ ...themes, general: v })} options={themesGeneral} />
                <SelectMulti label="Sport" value={themes.sport} onChange={(v) => setThemes({ ...themes, sport: v })} options={themesSport} />
                <SelectMulti label="Érotique" value={themes.erotic} onChange={(v) => setThemes({ ...themes, erotic: v })} options={themesErotic} />
                <SelectMulti label="Professionnel" value={themes.professional} onChange={(v) => setThemes({ ...themes, professional: v })} options={themesProfessional} />
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="custom">
            <Card>
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <SelectMulti label="Style d’écriture" value={customization.style} onChange={(v) => setCustomization({ ...customization, style: v })} options={[
                ...
                <SelectMulti label="Ton" value={customization.tone} onChange={(v) => setCustomization({ ...customization, tone: v })} options={tones} />
                <SelectMulti label="Types de mort" value={customization.death} onChange={(v) => setCustomization({ ...customization, death: v })} options={["Mort héroïque","Sacrifice","Trahison mortelle","Mort inattendue","Mort symbolique","Mort tragique","Mort mystérieuse","Mort en combat","Mort par maladie","Mort par accident"]} />
                <SelectMulti label="Types de sexe" value={customization.sex} onChange={(v) => setCustomization({ ...customization, sex: v })} options={["Scènes de séduction","Scènes d'amour","Scènes érotiques","Scènes de passion","Scènes de désir","Scènes de tentative","Scènes de rencontre","Scènes de relation interdite","Scènes de relation complexe"]} />
                <SelectMulti label="Phrases d’ouverture" value={customization.opening} onChange={(v) => setCustomization({ ...customization, opening: v })} options={["Mystérieuse","Intrigante","Dramatique","Poétique","Humouristique","Émotionnelle","Suspense","Réfléchie","Descriptive","Inspirante"]} />
